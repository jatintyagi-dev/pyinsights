...
def get_date(df):
    for label, content in df.items():
        if is_datetime64_any_dtype(content):
            st.write(
                f'Datetime feature - {label} - converted to multiple separate columns like date, month, year..')
...
def spear_C(df):
    if df.shape[0] > 30000:
        ...
        sp = np.round(scipy.stats.spearmanr(
            df_tmp, nan_policy='omit').correlation, 4)
    else:
        sp = np.round(scipy.stats.spearmanr(
            df, nan_policy='omit').correlation, 4)
    sp_con = hc.ward(sp)
