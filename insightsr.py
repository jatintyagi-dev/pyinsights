...
st.set_page_config(page_title="BDB insights", page_icon=":shark:",
                   layout="wide", initial_sidebar_state="expanded")

set_page_title("BDB insights")
...
prodheader = "<div><span class='fontblue'><span class='bold'>BDB Insights</span></div>"
...
if df is not None:
    ...
    target_variable = st.sidebar.selectbox(
        'select Target column (numeric) for insights', (df_raw.columns))
    ready = st.sidebar.button("Generate Insights")
    st.sidebar.header('2. Key Insights')
    st.markdown("<div><span class='fontblue'><span class='bold'>About BDB Insights</span></div>",
                unsafe_allow_html=True)
    with st.beta_expander("Insights provided and how to interpret", expanded=True):
        st.markdown("Once the user uploads the data to be analysed and provides the target column against which insights to be provided, BDB Insights tool will handle **pre-processing of data** which includes removal of null columns, highly correlated columns, columns that has more than 75% null values, identifies and  converts date time columns to multiple columns that helps in providing better insights. This pre-processing step is covered in <span class='fontcoral'><b>Section 2.1</b></span>", unsafe_allow_html=True)
        st.write("After pre-processing, BDB Insights tool will statistically analyse the dataset using machine learning algorithms and provides insights below:-")
        st.markdown(f"* Provides **top contributing features against the target data column**. For eg) if target is 'Sales' features that might turn important is price, seasonality etc.  BDB Insights tool provides such highly important features and % of contribution. This is covered in <span class='fontcoral'><b>Section 2.2</b></span>", unsafe_allow_html=True)
        st.markdown(f"* Provides **data visualizations for top 5 contributing features**. BDB Insights tool puts numeric features into multiple buckets to provide a different perception by aligning data into bucketed categories. This is covered in <span class='fontcoral'><b>Section 2.3</b></span>", unsafe_allow_html=True)
        st.markdown(f"* In <span class='fontcoral'><b>Section 2.4</b></span>, deeper insights are provided based on identified top contributors. Based on a sample picked from the top contributors, BDB Insights first provides **info on whether the contribution is +ve or -ve and by what %**. Features that brought down/up the mean target value is an immensely useful insight. Further within the sample how did each feature contribute is detailed.", unsafe_allow_html=True)
    ...
    if ready:
        ...
        rows = df_raw.shape[0]
        cols = df_raw.shape[1]
        with st.beta_expander("Few stats regarding the dataset"):
            st.write(
                f"Number of records: {rows}, Number of Features/Columns: {cols}")
        ...
        # spear_C(df_raw)
        with st.beta_expander("Highly Correlated Cols"):
            st.write(
                "Columns that are highly correlated with each other are depicted in the below figure (light shaded columns)")
        ...
        # with st.beta_expander("Added/Removed Columns that aids in generating better insights"):
        ...
        with st.beta_expander("Removed high cardinal columns and Null records"):
        ...
        with st.beta_expander("Date Columns being converted"):
            get_date(df_raw)

        with st.beta_expander("Display sample records from the pre-processed dataset"):
            st.write(df_raw.astype('object').head())

        df_trn, y_trn, nas = proc_df(
            df_raw, target_variable, max_n_cat=10, subset=50000)
        ...
        features, model = get_features(score, oob, trn_cols, trn_rows, df_trn, y_trn, n_estimators1,
        ...
        st.sidebar.markdown(
            f"<div><span class='teal'>Important contributors to {target_variable}</span></div>", unsafe_allow_html=True)
        ...
        st.markdown(
            f"<div><span class='fontgreen'><span class='bold'>2.2 Data Columns that are Important contributors towards {target_variable}</span></div>", unsafe_allow_html=True)
        ...
        with st.beta_expander("Additional Details"):
            ...
            st.write(
                f"Above graph depicts important data columns that were contributing to the target: {target_variable}")
            st.write(
                f"Details relating how were those features were impacting the {target_variable}, tontinued below..")
        ...
        topfeatures = [features['features'].iloc[0], features['features'].iloc[1], features['features'].iloc[2],
                       features['features'].iloc[3], features['features'].iloc[4], features['features'].iloc[5], features['features'].iloc[6]]
        ...
        with st.beta_expander("Top 1st Important Contributor"):
            maxslice1, minslice1 = display_visuals(
                df_raw, df_trn, feats1, imp1, attr1, target_variable, tmean)
        ...
        with st.beta_expander("Top 2nd Important Contributor"):
            maxslice2, minslice2 = display_visuals(
                df_raw, df_trn, feats2, imp2, attr2, target_variable, tmean)

        with st.beta_expander("Top 3rd Important Contributor"):
            maxslice3, minslice3 = display_visuals(
                df_raw, df_trn, feats3, imp3, attr3, target_variable, tmean)

        with st.beta_expander("Top 4th Important Contributor"):
            maxslice4, minslice4 = display_visuals(
                df_raw, df_trn, feats4, imp4, attr4, target_variable, tmean)

        with st.beta_expander("Top 5th Important Contributor"):
            maxslice5, minslice5 = display_visuals(
                df_raw, df_trn, feats5, imp5, attr5, target_variable, tmean)
        ...
        def print_contribs(bias, prediction, feats):
            ...
            st.write(
                f"Global Mean Contribution for {target_variable} is {round(bias,2)}")
            st.write(
                f"Mean Prediction from this sample is {round(prediction,2)}")
            s_preds = prediction - bias
            if s_preds > 0:
                st.markdown(
                    f"<div><span class='green'><span class='bold'>Contribution from this sample is {round(s_preds,2)}</span></div>", unsafe_allow_html=True)
            else:
                st.markdown(
                    f"<div><span class='fontred'><span class='bold'>Contribution from this sample is {round(s_preds,2)}</span></div>", unsafe_allow_html=True)
            # with st.beta_expander("Display all contributions from various features for the MAX sample"):
        ...
        st.markdown(
            "<div><span class='fontgreen'><span class='bold'>2.4 Feature contributions to the Target</span></div>", unsafe_allow_html=True)

        bias, prediction, df_tree, t_conts, l_conts, sample = get_contributions(
            maxslice1, model, df_trn)
        mstmp = sample[topfeatures].describe(include='all').T
        st.markdown(
            f"<div><span class='fontgrey'>1. Max category from 1st important contributor: {feats1} </span></div>", unsafe_allow_html=True)
        ...
        with st.beta_expander("Details of Indv contributions towards the mean prediction target"):
            ...
            fig = go.Figure(go.Waterfall(
                ...
                connector={"mode": "between", "line": {
                    "width": 4, "color": "rgb(0, 0, 0)", "dash": "solid"}}
            ))
        ...
        bias, prediction, df_tree, t_conts, l_conts, sample = get_contributions(
            maxslice2, model, df_trn)
        mstmp = sample[topfeatures].describe(include='all').T
        # st.write('\033[95m' + "*"*100 + '\033[0m')
        st.markdown(
            f"<div><span class='fontgrey'>2. Max category from 2nd important contributor: {feats2}</span></div>", unsafe_allow_html=True)
        with st.beta_expander("About this data sample"):
            st.write(mstmp)
        ...
        # Commented out IPython magic to ensure Python compatibility.
        bias, prediction, df_tree, t_conts, l_conts, sample = get_contributions(
            maxslice3, model, df_trn)
        mstmp = sample[topfeatures].describe(include='all').T
        st.markdown(
            f"<div><span class='fontgrey'>3. Max category from 3rd important contributor: {feats3}</span></div>", unsafe_allow_html=True)
        with st.beta_expander("About this data sample"):
            st.write(mstmp)
        ...
        bias, prediction, df_tree, t_conts, l_conts, sample = get_contributions(
            maxslice4, model, df_trn)
        mstmp = sample[topfeatures].describe(include='all').T
        st.markdown(
            f"<div><span class='fontgrey'>4. Max category from 4th important contributor: {feats4}</span></div>", unsafe_allow_html=True)
        with st.beta_expander("About this data sample"):
            st.write(mstmp)
        ...
        bias, prediction, df_tree, t_conts, l_conts, sample = get_contributions(
            maxslice5, model, df_trn)
        mstmp = sample[topfeatures].describe(include='all').T
        st.markdown(
            f"<div><span class='fontgrey'>5. Max category from 5th important contributor: {feats5}</span></div>", unsafe_allow_html=True)
        with st.beta_expander("About this data sample"):
            st.write(mstmp)
